...
class Account:
                ...
                while not amount:
                    try:
                        ...
                        if amount < 0:
                            ...
                            if amount > 0 and self.withdraw(
                                    int(token_selection) * amount):
                                for i in range(amount):
                                    self.tokens[token_selection].append(
                                        Token(token_selection,
                                              int(token_selection)))
            ...
            if tokens_available:
                ...
                while selection != len(options) - 1:
                    ...
                    if amount >= 0:
                        ...
                        token = self.tokens[options[selection].split(' ')[1]
                                            [1:]].pop()
                        bet.append(token)
                ...
                if len(bet) != 0:
                    print(
                        f"{self.name} is betting {len(bet)}x tokens valued at ${value}."
                    )
                    return True
    ...
    def menu(self):
        ...
        '''
        options = [
            "[i] Details", "[d] Deposit", "[e] Exchange", "[r] Ready",
            "[q] Quit"
        ]
        options_dict = {
            0: 'Details',
        ...
        while selection != "Ready" and selection != "Quit":
            if selection == "Details":
                ...
                deposit_options = [
                    "[1] $1.00", "[2] $5.00", "[3] $10.00", "[4] $20.00",
                    "[5] $100.00", "[r] Return"
                ]
                deposit_options_dict = {
                    0: 1,
                    1: 5,
                    2: 10,
                    3: 20,
                ...
                deposit_menu = TerminalMenu(deposit_options,
                                            title="\n- Deposit -\n")
                ...
                exchange_options = [
                    "[b] Buy Tokens", "[s] Sell Tokens", "[r] Return"
                ]
                exchange_options_dict = {0: 'Buy', 1: 'Sell', 2: "Back"}

                exchange_menu = TerminalMenu(exchange_options,
                                             title="\n- Exchange -\n")
                exchange_selection = exchange_options_dict[
                    exchange_menu.show()]
    ...
    def __str__(self):
        ...
        for t in self.tokens:
            if len(self.tokens[t]) > 0:
                tokens += (
                    f"¢{t} {len(self.tokens[t])}x ${int(t) * len(self.tokens[t])}, "
                )
